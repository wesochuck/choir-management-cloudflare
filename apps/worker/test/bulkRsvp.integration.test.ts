    ...
    database
      .prepare(
        `INSERT INTO member (id, organizationId, userId, role, createdAt)
         VALUES (?, ?, 'bulk-rsvp-manager', 'admin', ?)`,
      )
...
async function signIn(): Promise<string> {
  await exports.default.fetch(
    api("alpha.localhost", "/api/auth/email-otp/send-verification-otp", undefined, {
      body: JSON.stringify({ email: MANAGER_EMAIL, type: "sign-in" }),
      headers: { "content-type": "application/json" },
      method: "POST",
    }),
  );
  const otp = readCapturedPlatformEmailsForTest()
    .find(
      (message) => message.kind === "email-one-time-code" && message.recipient === MANAGER_EMAIL,
    )
    ?.text.match(/Use (\d{6}) to sign in/)?.[1];
  const response = await exports.default.fetch(
    api("alpha.localhost", "/api/auth/sign-in/email-otp", undefined, {
      body: JSON.stringify({ email: MANAGER_EMAIL, otp }),
      headers: { "content-type": "application/json" },
      method: "POST",
    }),
  );
  ...
  await database
    .prepare(
      `INSERT INTO user
       ...
       VALUES ('bulk-rsvp-manager', 'Bulk RSVP Manager', ?, 0, ?, ?, 0)`,
    )
    .bind(MANAGER_EMAIL, now, now)
    .run();
