...
interface PlatformSchemaStatusRow {
...
router.get("/api/platform/setup-status", async (context) => {
  ...
  const checks: PlatformSetupStatusResponse["checks"] = [
    ...
    },
    {
      ...
      status: jobDeadLetterCount === null ? "error" : jobDeadLetterCount === 0 ? "ok" : "attention",
    },
    {
      detail:
        schemaStatus === null
...
function seasonMutationFailure(
  error: unknown,
  requestIdValue: string,
  fallback: string,
...
async function saveSeasonRoute(context: Context<WorkerHonoEnvironment>, seasonId: string | null) {
  const authorization = await authorizeCalendarRoute(context, true);
  if (!authorization.ok) {
    return context.json(
      { ...authorization, requestId: context.get("requestId") },
