  ...
  for (const row of storage.sql
    .exec<CommerceContactRow>(
      `SELECT c.id, c.display_name AS displayName, c.email, c.phone,
       ...
       WHERE c.id IN (${placeholders(unique.length)})`,
      ...unique,
    )
    .toArray()) {
    rows.set(row.id, row);
  ...
  for (const row of storage.sql
    .exec<CommerceContactRow & { readonly normalizedEmail: string | null }>(
      `SELECT c.id, c.display_name AS displayName, c.email, c.phone,
       ...
       LEFT JOIN contact_communication_preferences sms_pref
         ON sms_pref.contact_id = c.id AND sms_pref.channel = 'sms'
       WHERE c.normalized_email IN (${placeholders(unique.length)})`,
      ...unique,
    )
    .toArray()) {
...
function readCommerceSelections(storage: AudienceStorage, audience: Audience): CommerceSelection[] {
    ...
    const ticketRows = storage.sql
      .exec<CommerceTransactionRow>(
        `SELECT p.id, p.buyer_name AS buyerName, p.buyer_email AS buyerEmail,
         ...
         ORDER BY p.buyer_name COLLATE NOCASE, p.id LIMIT ${String(AUDIENCE_COMMERCE_LIMIT)}`,
        audience.eventId ?? "",
        audience.eventId ?? "",
        audience.eventId ?? "",
        audience.eventId ?? "",
      ...
      .toArray();
    for (const row of ticketRows) selections.push({ row, voicePart: "Ticket Buyer" });
  }
  if (audience.targetAudiences.includes("Donors")) {
    const donorRows = storage.sql
      ...
      .toArray();
    for (const row of donorRows) selections.push({ row, voicePart: "Donor" });
  }
  return selections;
}
...
function toCommerceCandidate(
  ...
  const normalized = normalizeCommerceEmail(destination.email);
  if (normalized !== null) {
  ...
  return {
    ...
    emailUnsubscribed: contact.emailStatus === "unsubscribed",
...
function commerceRecipients(storage: AudienceStorage, audience: Audience): AudienceCandidate[] {
  const selections = readCommerceSelections(storage, audience);
  if (selections.length === 0) return [];
  // Each audience was built independently; cross-audience destination
  // dedupe happens in the caller merge step (Map-keyed with suppression
  // precedence), never via shared in-store seen sets. Within one commerce
  // audience, duplicate buyer emails still collapse to a single candidate.
  ...
  const seenDonorEmails = new Set<string>();
  for (const selection of selections) {
    const contact = resolveCommerceContact(selection, resolved);
    if (!contact) continue;
    const candidate = toCommerceCandidate(
      selection,
      contact,
      resolved.linkedFlags,
      selection.voicePart === "Ticket Buyer" ? seenBuyerEmails : seenDonorEmails,
    );
 ...
 * One batched IN query per table (never per-contact N+1); unknown IDs yield
 * no flags. Contact preference state is read from the candidate row itself.
 */
  ...
  const profileRows = storage.sql
    .exec<{
      ...
      `SELECT id, do_not_email AS doNotEmail,
       ...
       FROM profiles WHERE id IN (${placeholders(unique.length)})`,
      ...unique,
    )
    .toArray();
  ...
  const suppressionRows = storage.sql
    .exec<{
      ...
      `SELECT profile_id AS profileId, channel FROM communication_suppressions
       WHERE active = 1 AND profile_id IN (${placeholders(unique.length)})`,
      ...unique,
    )
    .toArray();
...
export async function resolveCommunicationAudienceFromStore(
  ...
  const recipients: AudienceCandidate[] = [
    ...
    ...commerceRecipients(storage, audience),
  ];
