...
import { acceptOrganizationInvitation, getOrganizationInvitation } from "./api";

type InvitationState =
  | { readonly status: "error" }
  | { readonly status: "loading" }
  | { readonly invitation: OrganizationInvitationDetails; readonly status: "ready" }
  | { readonly organizationName: string; readonly status: "success" };
...
export function AcceptInvitationView({ invitationId }: { readonly invitationId: string | null }) {
  ...
  async function acceptInvitation() {
    ...
    try {
      ...
      setState({ organizationName: state.invitation.organizationName, status: "success" });
    } catch {
      setErrorMessage(
    ...
    return (
      <main className="auth-layout">
        <section className="auth-card" aria-labelledby="invitation-accepted-title">
          ...
          <h1 id="invitation-accepted-title">You joined {state.organizationName}.</h1>
