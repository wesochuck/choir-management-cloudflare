...
test("organization administrator is routed directly to /admin upon login on an organization host", async ({
  ...
  await page.route("**/api/platform/security/status", async (route) => {
    await route.fulfill({
      body: JSON.stringify({ activePlatformAdministrator: false }),
      contentType: "application/json",
      status: 200,
    });
  });
  ...
  await page.route("**/api/organization/roster/configuration", async (route) => {
    await route.fulfill({
      body: JSON.stringify({ performerLabel: "Performer" }),
      contentType: "application/json",
      status: 200,
    });
  });
  await page.route("**/api/organization/branding", async (route) => {
    await route.fulfill({
      body: JSON.stringify({ logoFileId: null, organizationName: "Alpha Choir" }),
      contentType: "application/json",
      status: 200,
    });
  });
